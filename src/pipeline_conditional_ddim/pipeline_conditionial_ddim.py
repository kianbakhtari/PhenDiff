...
class ConditionalDDIMPipeline(DiffusionPipeline):
    ...
    def _new_call_for_paired_training(
        ...
        if isinstance(self.unet.module.config.sample_size, int):
            image_shape = (
                batch_size,
                self.unet.module.config.in_channels,
                self.unet.module.config.sample_size,
                self.unet.module.config.sample_size,
            )
        else:
            image_shape = (
        ...
        for t in timesteps:
            ...
            if do_classifier_free_guidance:
                ...
                uncond_output = self.unet(
                    ...
                    timestep=t,
                    class_labels=None,
                    class_emb=torch.zeros((batch_size, self.unet.module.time_embed_dim)).to(
                        device
                    ),
                ).sample
    ...
    @torch.no_grad()
    def _new_call_for_paired_testing(
        self,
        class_labels: torch.Tensor | None,
        class_emb: torch.Tensor | None = None,
        w: int | float | torch.Tensor | None = None,
    ...
    def __call__(
        ...
        if isinstance(self.unet, DistributedDataParallel):
            if for_testing:
                return self._new_call_for_paired_testing(
                class_labels=class_labels,
                class_emb=class_emb,
                w=w,
                generator=generator,
